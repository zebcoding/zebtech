...
import uk.co.zebcoding.zebtech.blocks.ZMachine;
import uk.co.zebcoding.zebtech.crafting.ZechoriumInfuserRecipes;
import uk.co.zebcoding.zebtech.network.PacketHandler;
import uk.co.zebcoding.zebtech.network.message.MessageTileEntityZechoriumFurnace;
import uk.co.zebcoding.zebtech.network.message.MessageTileEntityZechoriumInfuser;

public class TileEntityZechoriumFurnace extends TileEntityUsesZechorium implements
        ISidedInventory {
    ...
    public TileEntityZechoriumFurnace() {
        super();
    }
    ...
    public void updateEntity() {
        ...
        if (!this.worldObj.isRemote) {
            ...
            if (this.burnTime == 0 && this.canSmelt()) {
                if (this.tank.getFluidAmount() >= 50)
                    this.currentItemBurnTime = this.burnTime = 50;
            }
        ...
        if (flag != this.burnTime > 0) {
            flag1 = true;
            ZMachine.updateBlockState(
                    this.burnTime > 0 ? ZBlocks.zechoriumFurnaceActive : ZBlocks.zechoriumFurnaceIdle, this.worldObj, this.xCoord, this.yCoord,
                    this.zCoord);
        }
    ...
    public Packet getDescriptionPacket() {
        return PacketHandler.INSTANCE.getPacketFrom(new MessageTileEntityZechoriumFurnace(this));
    }
}
